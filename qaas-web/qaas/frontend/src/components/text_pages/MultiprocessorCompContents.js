...
export default function MultiprocessorCompContents() {
    return (
        <div className="textPageContainer">
            ...
            <p>
                Fig. bestcomp summarizes key points about these miniapp runs. This includes performance
                and the maximum number of cores for which


                {' '}
                <TooltipComponent id="efficiency-formula" content="Ec =t1/c tc">
                    efficiency
                </TooltipComponent>
                {' '}
                Ec &gt;.5, which ranges from 16 to 48. (footnote?? where Ec =t1/c tc) – in table bestcomp,
                the Ec threshold is adjustable [Yue]. It is interesting and typical to see the diversity
                of performance, winning compilers, and total cores usable efficiently. As examples,
                AMG is a multigrid solver with data access difficulties leading to poor relatively
                poor performance and low compiler differentiation. On the other hand, HACC
                is easily parallelizable and vectorizable so it runs well everywhere with
                little need for compiler excellence.
            </p>
            <p>
                Fig. bestcomp includes several performance metrics: the ICL Gf column (from Table unicore perf) leads to the ratio (Gfuni)/(GfMP/core) [ADD],
                which varies from 1.3 for HACC to 2.95 for Cloverleaf++. This is reasonable because MP computations
                contain extra parallelization instructions. Since HACC has high parallel efficiency we can expect
