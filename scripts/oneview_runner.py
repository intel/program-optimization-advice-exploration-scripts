...
def aggregate_loop_time(run_dir, binary, src_file, loop_head_line_num):
    ...
    return loop_profile.loc[matchedMask,LPROF_TIME_METRIC].sum().item()*1000 + 1, LPROF_TIME_METRIC
...
def run_lprof_loop_profile(run_dir, my_env, run_cmd, binary_name):
    ...
    true_run_cmd=f'{MAQAO_BIN} lprof --show-full-paths --display-raw-events -dl xp={LPROF_OUT_DIR} -- {true_run_cmd}'
    subprocess.run(true_run_cmd, shell=True, env=my_env, cwd=run_dir)

def parse_lprof_loop_profile(my_env, run_dir, binary):
    true_run_cmd = f'{MAQAO_BIN} lprof -dl -of=csv xp={LPROF_OUT_DIR}'
    subprocess.run(true_run_cmd, shell=True, env=my_env, cwd=run_dir)
