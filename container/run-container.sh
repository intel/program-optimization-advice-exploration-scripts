...
# NOTE1 rootless podman requires that process ID 0 runs as root (non-root on the host)
# NOTE2 Never mount host-specfic files/fs from the host to container => should run with the least resources possible
podman run --user=root --hostname $(hostname) --rm  ${mount_args[*]} ${env_args[*]} -v /:/host -v /lib/modules:/lib/modules -v /tmp/tmp:/tmp/tmp -v /dev:/dev --pid=host --ipc=host -w /host/$(pwd) -it local_image_qaas:latest 
