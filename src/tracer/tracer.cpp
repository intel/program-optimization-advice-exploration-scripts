...
void Tracer::visit(SgNode* n){
    ...
    if (isSgExprStatement(n)){
        ...
        SgFunctionCallExp* func_call = isSgFunctionCallExp(func_stmt->get_expression());
        if (func_call)
        {
            string func_name = func_call->getAssociatedFunctionDeclaration()->get_name();
...
void ParseLoopFiles::visit(SgNode *node){
    ...
    if (locatedNode){
        ...
        if (directives != NULL) {
            ...
            {
                std::string directiveTypeName = PreprocessingInfo::directiveTypeName((*i)->getTypeOfDirective()).c_str();
                if (directiveTypeName == "CpreprocessorIncludeDeclaration")
                {
                    ...
                    if (parseFileName ( &incFileName) == parseFileName ( &loopFileName))
                    {
...
void Tracer::initTracing() {
    ...
    for (int i = 0; i < LFI.size(); i++) {
        ...
        moveCodeletSourcefile(tracePrefixStr, current_lfi->getFuncName(), source_file_name, false);
        delete(tracePrj);
        ...
        string driverFileName = "restore_" + source_file_name;   //@@@ USED to be driver_...
        ...
        moveCodeletSourcefile("restore", current_lfi->getFuncName(), source_file_name, true);
        moveDriverFile("restore", current_lfi->getFuncName(), source_file_name);//@@@???
        
        // We unparse ast for SAVE source file after finishing the generation of a RESTORE source file,
        // because we need some data (globalscope, loopFuncNode, classes, ...@@@) from SAVE ast to generate the RESTORE source file.
        project->unparse(); 
        moveCodeletSourcefile(savePrefixStr, current_lfi->getFuncName(), source_file_name, false);
        delete(savePrj);
    ...
    /* Exporting visited function calls that were collected during CallGraph traversal 
     * to a file for later use in memory tracing pintool
     */
    exportFuncCalls();

}

void Tracer::traverseCG (SgGraphNode* CGNode, SgIncidenceDirectedGraph *CG, boost::unordered_map< SgFunctionDeclaration *, SgGraphNode * > &mapCG) {
    ...
    if (edge.size() > 0) {
        ...
        */
        ...
        {
            cout << "EDGE: \nNode A: " << (*it2)->get_node_A()->get_name() << "; Node B: " << (*it2)->get_node_B()->get_name() <<  endl;
...
void Tracer::exportFuncCalls() {
    ...
    for (auto it = visitedFuncCalls.begin(); it != visitedFuncCalls.end(); ++it) {
...
void Tracer::insertLoopFuncDecl() {
    ...
    if(exprStmt) {
        ...
        if(funcExpr) {
            SgFunctionDeclaration* funcDecl = funcExpr->getAssociatedFunctionDeclaration();
            if(funcDecl) {
                restoreOutfile << funcDecl->unparseToString() << endl;
            } else {
                cout << "Function Declaration can't be resolved statically!!! " << endl;
            }
        } else {
...
void Tracer::typeDeclaration(SgType* tp) {
    ...
    else if (SgClassType* classType = isSgClassType (tp))
    {
        ...
        {
            SgDeclarationStatement* typeDecl = classType->getAssociatedDeclaration ();
            if (SgClassDeclaration* classDecl = isSgClassDeclaration (typeDecl))
            {
                SgClassDeclaration* declStatOfClassDecl = isSgClassDeclaration (classDecl->get_definingDeclaration ());
                SgClassDefinition* classDef = declStatOfClassDecl->get_definition ();
                
                SgDeclarationStatementPtrList declStmtList = classDef->get_members();
                SgDeclarationStatementPtrList::iterator declIt = declStmtList.begin();
                SgDeclarationStatementPtrList::iterator declEnd = declStmtList.end();
                for (; declIt != declEnd; ++declIt)
                {
                    SgDeclarationStatement *declStmt = isSgDeclarationStatement(*declIt);
                    SgVariableDeclaration *varDecl = isSgVariableDeclaration(declStmt);
                    SgInitializedNamePtrList memVariables = varDecl->get_variables ();
                    for (int i = 0; i < memVariables.size (); i++)
                    {
                        SgInitializedName* memPtr = isSgInitializedName (memVariables[i]);
    ...
    else if (SgTypedefType* typedefType = isSgTypedefType (tp))
    {
        ...
        else 
        {
            if (SgType* baseType = typedefType->get_base_type ())
            {
                ...
                classDeclarations << "typedef " << baseType->unparseToString () << " " << tp->unparseToString () << ";" << std::endl;
            }
        }
    }
    ...
    {
        classDeclarations << enumType->get_declaration ()->unparseToString () << std::endl;
    }
    else 
    {
...
void Tracer::variableDeclaration(SgVariableSymbol* smbl) {
    ...
    SgDeclarationStatement* varDecl = smbl->get_declaration ()->get_declaration ();
...
// Stack allocation
    ...
    restoreOutfile << "unsigned long size = ((unsigned long long)preRSP) - ((unsigned long long) min_stack_address);\n";
    restoreOutfile << "printf(\"size=%d\\n\", size);\n";
    restoreOutfile << "void* ptr = alloca (size);\n";
    restoreOutfile << "printf(\"sp = %p\\n\", ptr);\n";
// Restore heap and stack
    restoreOutfile << "my_mallocMemoryChunkInclusive(\"myDataFile/test.hd\", min_heap_address, max_heap_address); \n";
    restoreOutfile << "// Restored heap.  Now safe to use heap, including fopen(), etc. \n";
    restoreOutfile << "readDataRangeFromFile(\"myDataFile/test.st\", min_stack_address, max_stack_address);\n";
...
void Tracer::insertGlobalVariablesRead() {
    ...
    for (int i = 0; i < globalNames.size (); i++)
    {
        ...
        if (smbl)
        {
            ...
            restoreOutfile << "fread ((void*)&" << globalNames[i] << ", sizeof (" << globalNames[i] << "), 1, fp);\n";
            
        }
        else 
        {
            std::cout << "NOT found variable with name " << globalNames[i] << std::endl;
        }
    }
...
void Tracer::insertLoopFuncCall() {
    ...
    if (exprStmt)
    {
        ...
        if (funcExpr)
        {
            ...
            {
                if (i > 0) restoreOutfile << ", ";
                auto funcArgument = funcArgs[i];
                SgVarRefExp* varArg = isSgVarRefExp (funcArgument);
                if (varArg)
                {
                    SgPointerType* ptrToVar = SageBuilder::buildPointerType (varArg->get_type ());
                ...
                else std::cout << "Argument is not a variable expression! " << std::endl;
            }
            restoreOutfile << ");" << std::endl;
            
            // @@@ Traverse thru parameters and insert restoration of pointers
        }
        else std::cout << "Something went wrong with funcExpr " << std::endl;
    }
    else std::cout << "SOMETHING WENT WRONG with exprStmt!" << std::endl;

    std::cout << "Inserting loop function call! " << std::endl;

}

void Tracer::moveDriverFile(string prefixStr, string funcName, string baseFileName) {
...
SgStatement* Tracer::buildDataSavingLines() {
    SageInterface::insertHeader("addresses.h", PreprocessingInfo::after, false, globalscope);
    ...
    basicBlockStmts.push_back (buildPrintFunc ("BP(STACK END): %09lx\\n", "basepointer"));
    basicBlockStmts.push_back (buildPrintFunc ("SP(STACK BEGIN): %09lx\\n", "((char*)stackpointer)-1")); 
    basicBlockStmts.push_back (buildPrintFunc ("DS START: %09lx\\n", "&__data_start"));
    basicBlockStmts.push_back (buildPrintFunc ("DS END: %09lx\\n", "((char*)&_end)-1"));                 
...
SgStatement* Tracer::buildWriteHeap(string datafileName) {
    ...
    SgVarRefExp* heap_arg1 = SageBuilder::buildVarRefExp (SgName ("min_heap_address"));
    SgVarRefExp* heap_arg2 = SageBuilder::buildVarRefExp (SgName ("max_heap_address"));
    SgVarRefExp* heap_arg3 = SageBuilder::buildVarRefExp (SgName ("heapPointerData"));
    ...
    SgExprStatement* callStmt = SageBuilder::buildFunctionCallStmt (SgName ("writeDataRangeToFile"), return_void_type, heap_arg_list);
    return callStmt;
}
...
SgStatement* Tracer::buildWriteStack(string datafileName) {
    ...
    SgVarRefExp* stack_arg1 = SageBuilder::buildVarRefExp (SgName ("min_stack_address"));
    SgVarRefExp* stack_arg2 = SageBuilder::buildVarRefExp (SgName ("max_stack_address"));
    SgVarRefExp* stack_arg3 = SageBuilder::buildVarRefExp (SgName ("stackPointerData"));
    ...
    SgExprStatement* callStmt = SageBuilder::buildFunctionCallStmt (SgName ("writeDataRangeToFile"), return_void_type, stack_arg_list);
    return callStmt;
}

void Tracer::buildHeaders() {
        SageInterface::insertHeader("unistd.h", PreprocessingInfo::after, false, globalscope);
        SageInterface::insertHeader("config.h", PreprocessingInfo::after, false, globalscope);
        SageInterface::insertHeader("util.h", PreprocessingInfo::after, false, globalscope);
        SageInterface::insertHeader("alloca.h", PreprocessingInfo::after, false, globalscope);
}

void Tracer::moveCodeletSourcefile(string prefixStr, string funcName, string baseFileName, bool isRestore){
    string dirPath = "./LoopExtractor_data/codelet_" + funcName + "/";
    string mkdirCommand = "mkdir -p " + dirPath;
    string result = executeCommand(mkdirCommand);
    if (result.find("failed") != string::npos) {
    ...
    /*
    string mvCommand = "mv " + prefixOfGeneratedSourcefile + baseFileName + " " + targetPath;
    result = executeCommand(mvCommand);
    if (result.find("failed") != string::npos) {
        cout << "mv FAILED!!!!" << endl;
    }
...
void Tracer::setLoopNode(SgNode* loopNode){
...
SgStatement* Tracer::buildVarDecl(std::string varName, std::string varType){
    SgType* brkType = SageBuilder::buildOpaqueType (varType, SageBuilder::topScopeStack ());
    SgVariableDeclaration* brkStmt = SageBuilder::buildVariableDeclaration (varName, brkType);
...
vector<SgStatement*> Tracer::buildSegmentPtrs()
{
    ...
    SgVariableDeclaration* basePtrStmt = SageBuilder::buildVariableDeclaration ("basepointer", voidPtrType, NULL, SageBuilder::topScopeStack());
    SgVariableDeclaration* stackPtrStmt = SageBuilder::buildVariableDeclaration ("stackpointer", voidPtrType, NULL, SageBuilder::topScopeStack());
    ...
    SageInterface::appendExpression (heap_arg_list, heap_arg1);
    SgFunctionCallExp* basePtrCall = SageBuilder::buildFunctionCallExp(SgName ("__builtin_frame_address"), voidPtrType, heap_arg_list);
    SgFunctionCallExp* stackPtrCall = SageBuilder::buildFunctionCallExp(SgName ("alloca"), voidPtrType, heap_arg_list);
    ...
    SgStatement *basePtrAsgn = SageBuilder::buildAssignStatement (basePtrRef, basePtrCall);
    SgStatement *stackPtrAsgn = SageBuilder::buildAssignStatement (stackPtrRef, stackPtrCall);
...
SgStatement* Tracer::buildPrintFunc(std::string printStr, std::string variableName){
    ...
    SgExprStatement* callStmt1 = SageBuilder::buildFunctionCallStmt (SgName("printf"), return_type, argBP_list);
...
std::vector <SgStatement* > Tracer::buildBrkStmt(){
    ...
    auto brkType = SageBuilder::buildOpaqueType ("char*", SageBuilder::topScopeStack ());
    SgFunctionCallExp* rhs = SageBuilder::buildFunctionCallExp ("sbrk", brkType, sbrkArgList);
...
std::vector <SgStatement* > Tracer::saveGlobalVars(){
    ...
    auto file_type = SageBuilder::buildPointerType (SageBuilder::buildOpaqueType ("FILE", SageBuilder::topScopeStack ()));
    std::string uniqueFileName = SageInterface::generateUniqueVariableName (SageBuilder::topScopeStack (), "fp");
    auto fileNameDecl = SageBuilder::buildVariableDeclaration (uniqueFileName, file_type , NULL, SageBuilder::topScopeStack ());
    ...
    SgFunctionCallExp* rhs = SageBuilder::buildFunctionCallExp ("fopen", file_type, fopen_arg_list, SageBuilder::topScopeStack ());
    ...
    {
        SgVariableSymbol* smbl = global_tbl->find_variable (SgName (globalVariableNames[i]));
        if (smbl != nullptr)
        {
            ...
            SageInterface::appendExpression (sizeArg, SageBuilder::buildVarRefExp(smbl));// smbl);
    ...
    SgExprStatement* fcloseStmt = SageBuilder::buildFunctionCallStmt (SgName ("fclose"), return_void_type, fclose_arg_list);
    ...
    if (exprStmt)
    {
        ...
        if (funcExpr)
        {
            ...
            if (numOfArgs > 0)
            {
                ...
                SgExprListExp* fopen_arg_list = SageBuilder::buildExprListExp ();
                SgStringVal* file_name = SageBuilder::buildStringVal ("saved_pointers.h");
                SgStringVal* file_modifier = SageBuilder::buildStringVal ("w");
                SageInterface::appendExpression (fopen_arg_list, file_name);
                SageInterface::appendExpression (fopen_arg_list, file_modifier);

                SgFunctionCallExp* rhs = SageBuilder::buildFunctionCallExp ("fopen", file_type, fopen_arg_list, SageBuilder::topScopeStack ());
                SgStatement *fopenStmt = SageBuilder::buildAssignStatement (lhs, rhs);
                ret.push_back (fopenStmt);
                ...
                for (int i = 0; i < numOfArgs; i++)
                {
                    ...
                    if (varArg) {
                        ...
                        fpline.append ("*)%ld \\n");
                        SgStringVal* lineVal = SageBuilder::buildStringVal (fpline);
                        SageInterface::appendExpression (fprintf_line_args, lineVal);
                        std::string varName = "&";
                    ...
                    else std::cout << "Argument is not a variable expression! \n" << "funcArgument: " << funcArgument->unparseToString () << std::endl;
                }
                SgExprListExp* fclose_arg_list = SageBuilder::buildExprListExp ();
                SgExpression* filePointer = SageBuilder::buildVarRefExp (fileNameDecl);
                SageInterface::appendExpression (fclose_arg_list, filePointer);
